...
glm::vec2 Util::clamp(glm::vec2 vec, float max)
{
	const float magSqr = squaredMagnitude(vec);
	if (magSqr > pow(max, 2))
	{
		float mag = sqrt(magSqr);
		float normalizedX = vec.x / mag;
		float normalizedY = vec.y / mag;
		return glm::vec2(normalizedX * max, normalizedY * max);
	}
	return vec;
}
